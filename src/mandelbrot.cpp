...
Color::Color(uint32_t val) : hue(0.0), saturation(0.0)
{
...
auto Color::to_rgb() const -> RGB
{
	...
	const auto c = RGB(round(components[0]), round(components[1]),
			   round(components[2]));

	// std::cout << round(components[0]) << " " << round(components[1]) << "
	// " << round(components[2]) << std::endl;
